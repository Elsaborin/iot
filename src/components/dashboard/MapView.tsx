...
const MapView: React.FC<MapViewProps> = () => {
  ...
  // Update markers when parcelas change
  useEffect(() => {
            ...
            <div class="space-y-2">
              ...
              <div class="grid grid-cols-2 gap-2">
                <div class="bg-blue-50 dark:bg-blue-900/20 p-2 rounded">
                  ...
                  <div class="font-bold text-blue-700 dark:text-blue-300">${parcela.sensor?.humedad.toFixed(1)}%</div>
                </div>
                <div class="bg-red-50 dark:bg-red-900/20 p-2 rounded">
                  <div class="text-red-600 dark:text-red-400 text-sm">Temperatura</div>
                  <div class="font-bold text-red-700 dark:text-red-300">${parcela.sensor?.temperatura.toFixed(1)}°C</div>
                </div>
                <div class="bg-cyan-50 dark:bg-cyan-900/20 p-2 rounded">
                  <div class="text-cyan-600 dark:text-cyan-400 text-sm">Lluvia</div>
                  <div class="font-bold text-cyan-700 dark:text-cyan-300">${parcela.sensor?.lluvia.toFixed(1)} mm</div>
                </div>
                <div class="bg-amber-50 dark:bg-amber-900/20 p-2 rounded">
                  <div class="text-amber-600 dark:text-amber-400 text-sm">Sol</div>
                  <div class="font-bold text-amber-700 dark:text-amber-300">${parcela.sensor?.sol.toFixed(1)}%</div>
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="h-[calc(100vh-170px)]">
      ...
      <div className="h-[calc(100%-2rem)] w-full bg-slate-100 dark:bg-slate-700 rounded-lg overflow-hidden">
